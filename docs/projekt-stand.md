...
- **DNS-Ausfall des Heimnetzes:** `deine-nextcloud.example.com` (split-horizon →
  ...
  **`extra_hosts: deine-nextcloud.example.com:10.0.10.100`** im `doday`-Service (steht jetzt in
...
- **CNAME-auf-IP im Pi-hole = Bug (14.06.2026):** Die CLIENT-Auflösung von
  `do.msmr.co`/`deine-nextcloud.example.com` brach am Heimnetz (v. a. iOS), weil im Pi-hole ein
...
flowchart LR
  ...
  subgraph NC["Nextcloud (deine-nextcloud.example.com)"]
